...
/**
 * Generate flight data from cache - NEVER uses mock data
 * This function only returns flights that were received from authentic sources
 */
function getCachedFlights(count: number = 300): LiveFlight[] {
  const flights = Array.from(flightCache.values());
  return flights.slice(0, count);
}

/**
 * Fetch flights based on the provided filter type (from authentic data sources only)
 */
